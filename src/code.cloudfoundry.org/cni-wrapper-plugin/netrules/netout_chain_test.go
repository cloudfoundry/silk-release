	...
	Describe("IPTablesRules", func() {
		...
		Context("when IPv6 is enabled", func() {
			...
			Context("when deny networks are specified", func() {
				...
				DescribeTable("it should deny the workload networks and the 'all' networks",
					func(workload string, denyNetworks netrules.DenyNetworks) {
						netOutChain.DenyNetworks = denyNetworks
						netOutChain.DenyNetworks.Always = []string{"2002:db8::/120"}

						iptablesRules, err := netOutChain.IPTablesRules("some-container-handle",
							workload, netrules.NewRulesFromGardenNetOutRules(netOutRules))
						Expect(err).NotTo(HaveOccurred())

						rulesWithDenyNetworksAndDefaults := append(
							genericRules,
							[]rules.IPTablesRule{
								{"-d", "2002:db8::/120", "--jump", "REJECT", "--reject-with", "icmp6-port-unreachable"},
								{"-d", "2001:db8::/120", "--jump", "REJECT", "--reject-with", "icmp6-port-unreachable"},
								{"-p", "tcp", "-m", "state", "--state", "INVALID", "-j", "DROP"},
								{"-m", "state", "--state", "RELATED,ESTABLISHED", "-j", "ACCEPT"},
							}...,
						)
					...
					Entry("when the workload is an app", "app", netrules.DenyNetworks{Running: []string{"2001:db8::/120"}}),
					Entry("when the workload is a task", "task", netrules.DenyNetworks{Running: []string{"2001:db8::/120"}}),
					Entry("when the workload is staging", "staging", netrules.DenyNetworks{Staging: []string{"2001:db8::/120"}}),
				)
				...
				DescribeTable(
					...
					func(workload string, expectedDenyNetwork string) {
						netOutChain.DenyNetworks = netrules.DenyNetworks{
							Always:  []string{"2001::/120"},
							Running: []string{"2002::/120"},
							Staging: []string{"2003::/120"},
						}
						...
						rulesWithDenyNetworksAndDefaults := append(
							genericRules,
							[]rules.IPTablesRule{
								{"-d", "2001::/120", "--jump", "REJECT", "--reject-with", "icmp6-port-unreachable"},
								{"-d", expectedDenyNetwork, "--jump", "REJECT", "--reject-with", "icmp6-port-unreachable"},
								{"-p", "tcp", "-m", "state", "--state", "INVALID", "-j", "DROP"},
								{"-m", "state", "--state", "RELATED,ESTABLISHED", "-j", "ACCEPT"},
							}...,
						)
					...
					Entry("when the workload is an app", "app", "2002::/120"),
					Entry("when the workload is a task", "task", "2002::/120"),
					Entry("when the workload is staging", "staging", "2003::/120"),
				)
			})
